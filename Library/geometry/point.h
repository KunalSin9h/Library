...
template<typename T> struct Point {
    ...
    // returns point rotated ’a’ radians ccw (counter clock wise) around the origin
    Point rotate(double a) const {
        return Point(x * cos(a) - y * sin(a), x * sin(a) + y * cos(a));
    };

    Point perp_cw() const { return Point(y, -x); } // clock-wise
    Point perp_ccw() const { return Point(-y, x); }// counter clock-wise

    T norm() const { return x * x + y * y; };

    auto abs() const { return sqrt(dist2()); };
